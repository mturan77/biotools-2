...
st.title("🧬 RNA-Seq Analiz Hattı (R Uyumlu Versiyon)")
...
def get_norm_counts(dds):
    # R'a en yakın sonuç için VST veya Log1p
    # log1norm varsa onu kullan, yoksa kendimiz hesaplayalım
    if hasattr(dds, 'layers') and 'log1norm' in dds.layers:
        ...
        # En kötü ihtimal: Size Factor normalizasyonu yapıp log alalım (Manuel DESeq2 mantığı)
        # Basitlik için log1p(X) kullanıyoruz ama üsttekiler genelde çalışır.
        data = np.log1p(dds.X)
...
with st.sidebar:
    ...
    ref_group = st.text_input("Referans Grup Adı", value="Control", help="Kontrol grubunun adı (Örn: Control, WT)")
...
if st.session_state.run_trigger:
    if not file_samples:
        ...
        try:
            ...
            with st.status("Analiz Yapılıyor... (Bu işlem veri boyutuna göre sürebilir)", expanded=True) as status:
                if file_hisat:
    ...
    if titles:
        ...
        for i, (method_name, dds) in enumerate(datasets):
            with tabs[i]:
                ...
                t1, t2, t3 = st.tabs(["📊 PCA (R Benzeri)", "🌋 Volcano", "🔥 Heatmap"])
                
                # --- 1. PCA (R MANTIĞIYLA DÜZELTİLDİ) ---
                with t1:
                    # 1. Varyansı Hesapla
                    variances = norm_counts.var(axis=0)
                    # 2. En yüksek varyansa sahip 500 geni seç (R varsayılanı)
                ...
                with t2:
                    ...
                    # R'daki mantık: Alfabetik değil, seçilen referansa göre
                    # Varsayılan olarak Test Grubu ilk, Ref grup ikinci olsun
                    def_idx_ref = 0
                    if ref_group in grps:
                         def_idx_ref = list(grps).index(ref_group)
                    
                    # Kullanıcıya Ref grubu otomatik seçtirme, diğerini test yapma
                    ...
                    if st.button(f"Karşılaştır: {g_test} vs {g_ref}", key=f"b_{method_name}"):
